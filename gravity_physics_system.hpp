...
class GravityPhysicsSystem {
   public:
    ...
    void update(std::vector<PveGameObject>& objs, float dt, unsigned int substeps = 1) {
    ...
    glm::vec2 computeForce(PveGameObject& fromObj, PveGameObject& toObj) const {
   ...
   private:
    void stepSimulation(std::vector<PveGameObject>& physicsObjs, float dt) {
...
class Vec2FieldSystem {
   public:
    void update(const GravityPhysicsSystem& physicsSystem,
                std::vector<PveGameObject>& physicsObjs,
